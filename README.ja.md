...
相対パス（例：`./subdir`、`../sibling`）を指定した場合は、登録時のカレントディレクトリを基準に自動で絶対パスへ変換されます。
...
#### cd の代わりにファイルエクスプローラーで開く

```sh
j <名前> e
```

`e` を追加すると、`cd` する代わりにファイルエクスプローラー（Windows はエクスプローラー、macOS は Finder、Linux は `xdg-open`）でディレクトリを開きます。
